defmodule Day04 do
  ...
  def stricter_valid?(password) when is_integer(password) do
    ...
    length(digits) == 6 && at_least_one_duplicate_digit?(digits) &&
      monotonically_increasing_digits?(digits)
  end
  ...
  defp monotonically_increasing_digits?([a, b | rest]) when a <= b,
    do: monotonically_increasing_digits?([b | rest])

  defp monotonically_increasing_digits?(_), do: false

  defp at_least_one_duplicate_digit?(digits) do
    digit_counts =
      Enum.reduce(digits, Map.new(), fn d, acc ->
