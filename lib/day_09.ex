defmodule Day09 do
  ...
  defmodule IntcodeState do
    ...
    def value_of(state, location, :relative_mode),
      do: Map.get(state.code, state.relative_base + location, 0)
  ...
  defp eval(:add, {mode1, mode2, mode3}, [arg1, arg2, arg3 | _], state) do
    new_value =
      IntcodeState.value_of(state, arg1, mode1) + IntcodeState.value_of(state, arg2, mode2)

    address = IntcodeState.write_address(state, arg3, mode3)
    new_code = IntcodeState.update_code_at(state, address, new_value)

    %{state | code: new_code, instruction_pointer: state.instruction_pointer + 3}
  end
  ...
  defp eval(
         ...
         {mode, _, _},
         [arg | _],
         state = %IntcodeState{inputs: [input | new_inputs]}
       ) do
  ...
  defp eval(:less_than, {mode1, mode2, mode3}, [arg1, arg2, arg3 | _], state) do
    new_value =
      if IntcodeState.value_of(state, arg1, mode1) < IntcodeState.value_of(state, arg2, mode2),
        do: 1,
        else: 0
    ...
    %{state | instruction_pointer: state.instruction_pointer + 3, code: new_code}
  end
