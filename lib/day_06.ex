defmodule Day06 do
  ...
  def orbital_transfers(raw_orbits, target_orbiting_with) do
    ...
    orbited_by_map =
      Enum.reduce(orbits_map, %{}, fn {o, com}, map ->
  ...
  defp dfs(target, _orbits_map, _orbited_by_map, [{target, visited} | _queue]),
    do: Enum.reverse(visited)
  ...
  defp dfs(target, orbits_map, orbited_by_map, [{current, visited} | queue]) do
    ...
    next_orbits =
      MapSet.difference(MapSet.new(potential_orbits), MapSet.new(visited)) |> MapSet.to_list()
